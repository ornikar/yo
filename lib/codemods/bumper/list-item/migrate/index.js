 ...
 *
 * Bumper requires an explicit 3-slot model — `<ListItem.Left>`,
 * `<ListItem.Content>`, `<ListItem.Right>` — and silently ignores any
 ...
 * `left={x}` prop into a `<ListItem.Left>` slot and the `right={x}` prop
 * into a `<ListItem.Right>` slot, keeps a source `<ListItem.Content>` as-is,
...
const REVIEW_MESSAGES = {
  ...
  multipleLeftSideContent:
    '[ListItem migration] Multiple left-side `ListItem.SideContent` elements were combined inside a single `<ListItem.Left>`. Bumper honors only one left slot — verify the layout.',
...
module.exports = async function transformer(fileInfo, api) {
  ...
  root.find(j.JSXElement).forEach((path) => {
    ...
    for (let i = 0; i < children.length; i += 1) {
      ...
      if (isMemberJsx(c, listItemLocalName, 'Left')) continue;
      if (isMemberJsx(c, listItemLocalName, 'Right')) continue;
    ...
    for (let i = 0; i < children.length; i += 1) {
      ...
      if (isMemberJsx(child, listItemLocalName, 'Left')) {
        for (const inner of (child.children || []).filter((c) => !isWhitespaceText(c))) {
      ...
      if (isMemberJsx(child, listItemLocalName, 'Right')) {
        for (const inner of (child.children || []).filter((c) => !isWhitespaceText(c))) {
    ...
    if (leftFromChildren.length > 0) {
      ...
      newChildren.push(buildSlot(j, listItemLocalName, 'Left', leftFromChildren));
    }
    ...
    if (rightFromChildren.length > 0) {
      newChildren.push(j.jsxText('\n'));
      newChildren.push(buildSlot(j, listItemLocalName, 'Right', rightFromChildren));
    }
