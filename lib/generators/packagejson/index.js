      ...
      packageLocations.forEach((packageLocation) => {
        ...
        if (pkg?.devDependencies?.['@ornikar/rollup-config']) {
          if (wpkg.private) {
            ...
            const rollupNodeVersion = '22.22';

            // metro platform builds are resolved via exports['react-native'] conditions (ios, android, default)
            if (!entries || entries.length === 0 || entries.length === 1) {
              delete wpkg.browser;
            } else {
            ...
            entries.forEach((entryName) => {
              ...
              wpkg.exports[entryPath] = {
                ...
                'react-native': hasPeerDependencyReactNative
                  ? {
                      jest: `./dist/${entryName}-node-${rollupNodeVersion}.cjs.js`,
                      ios: `./dist/${entryName}-metro.es.ios.js`,
                      android: `./dist/${entryName}-metro.es.android.js`,
                      default: `./dist/${entryName}-metro.es.ios.js`,
                    }
                  : {
                      jest: `./dist/${entryName}-node-${rollupNodeVersion}.cjs.js`,
                      default: `./dist/${entryName}-node-${rollupNodeVersion}.cjs.js`,
                    },
                browser: {
